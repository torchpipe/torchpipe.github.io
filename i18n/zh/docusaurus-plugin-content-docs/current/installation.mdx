...
| `tensorrt`(可选) | >= 7.2(Starting from 0.3.2rc3)<br /><= 8.6.1.5 | - tensorrt 7.0 动态输入下存在[内存泄漏](https://github.com/NVIDIA/TensorRT/issues/351)<br />- tensorrt 7.1 及以下无法直接int8化动态输入的模型 |
...
| BUILD_PPLCV      |   0    |         是否编译依赖ppl.cv的c++后端          |
