...
详细可参见[示例](https://g.hz.netease.com/deploy/torchpipe/-/tree/develop/examples/int8).
...
我们集成了[calib_tools](https://g.hz.netease.com/deploy/torchpipe/-/blob/develop/examples/int8/qat/calib_tools.py)，可做参考.
...
- 下载训练数据：[代码](https://g.hz.netease.com/deploy/torchpipe/-/blob/develop/examples/int8/qat/download_data.py)
- 训练10个epoch获得resnet50模型：[代码](https://g.hz.netease.com/deploy/torchpipe/-/blob/develop/examples/int8/qat/fp32_train.py), 精度98.44%
- (optinal)pytorch ptq：[代码](https://g.hz.netease.com/deploy/torchpipe/-/blob/develop/examples/int8/qat/ptq.py), 精度96.64%（max）
- (optinal)pytorch qat：[代码](https://g.hz.netease.com/deploy/torchpipe/-/blob/develop/examples/int8/qat/qat.py), 精度98.26%. 

#### mse + 残差融合 {#mseadd}

以上resnet的训练，采用max方式量化，并且没有对Add进行融合，导致tensorrt运行速度未达预期。以下将Add在int8下进行融合并换用mse模式后的结果：

- ptq：[代码](https://g.hz.netease.com/deploy/torchpipe/-/blob/develop/examples/int8/qat/ptq_merge_residual.py), 精度94.34%（mse）
- qat：[代码](https://g.hz.netease.com/deploy/torchpipe/-/blob/develop/examples/int8/qat/qat_merge_residual.py), 精度95.82%
...
- 导出onnx：[代码](https://g.hz.netease.com/deploy/torchpipe/-/blob/develop/examples/int8/qat/export_onnx_merge_residual.py)
- 使用torchpipe加载fp32-onnx并进行ptq： [代码](https://g.hz.netease.com/deploy/torchpipe/-/blob/develop/examples/int8/qat/torchpipe_ptq_test.py)
- 使用torchpipe加载qat-onnx进行测试： [代码](https://g.hz.netease.com/deploy/torchpipe/-/blob/develop/examples/int8/qat/torchpipe_qat_test.py)
...
[onnxruntime下精度]: https://g.hz.netease.com/deploy/torchpipe/-/blob/develop/examples/int8/qat/onnxruntime_qat_test.py