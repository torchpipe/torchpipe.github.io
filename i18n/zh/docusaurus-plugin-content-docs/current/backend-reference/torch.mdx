...
| [SyncTensor](#synctensor)|`device_id` `SyncTensor::backend` |data[at::Tensor] |result[at::Tensor]   ||
...
- device_id: 默认为-1；表示将当前设备设置为这个id. 内部相当于调用了`c10::cuda::set_device(device_id)` 或者 `torch.cuda.set_device(device_id)`.
- 使用方式：
...
## Torch
[SyncTensor](#synctensor)的别名。 从0.3.1b2版本开始生效。
