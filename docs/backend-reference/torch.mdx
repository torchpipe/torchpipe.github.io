...
| [SyncTensor](#synctensor)|`SyncTensor[]` |data[any] |result[any]   ||
| [Torch](#torch)|`device_id` `Torch[]` |data[at::Tensor/vecotr<at::Tensor>] |result[same as data]   |从0.3.2b0开始生效|
...
## PillowResizeTensor
- 输入的`at::Tensor`，元素类型需要是`at::kByte`，形状为13hw
- resize_h 和 resize_w 需要为整数，有效范围为[1, 1024 * 1024]
...
- SyncTensor会在初始化时判断当前线程是否[绑定到了默认流](../preliminaries/pytorch_libtorch)。是的话，将激活自身功能：在初始化时将该线程切换到独立的流，并在前向时进行流同步。
...
- Torch[]: 必须提供
- device_id: 默认为-1；表示不切换设备. 内部相当于调用了`c10::cuda::set_device(device_id)` 或者 `torch.cuda.set_device(device_id)`.
- 使用方式：
    - `Torch[BackendTensor]`
- 在pytorch环境直接使用（不经过调度后端）时，本后端将不生效，以便兼容[pytorch cuda语义](../preliminaries/pytorch_libtorch)。经过默认调度后端时，初始化和前向可认为在同一个独立线程中完成。
...
|precision::fp32 | 设置部分层的精度为fp32（覆盖precision的设置）| 层的名称，以逗号分开(建议将检测/分类最后一层设置为fp32或者fp16)|
|precision::fp16 | 设置部分层的精度为fp16（覆盖precision的设置）| 层的名称，以逗号分开(建议将检测/分类最后一层设置为fp32或者fp16)|
...
|-|-|-|
|cpu|将数据拷贝到cpu上||
|softmaxcpu|对二维tensor进行softmax操作后，将数据拷贝到cpu上||

以下是`softmaxcpu`的参考实现：
```cpp
